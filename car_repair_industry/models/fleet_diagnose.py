...
class SparePartLine(models.Model):
	...
	part_line_image = fields.Binary(string="Image", related="product_id.image_1920")
	...
	def _onchange_product_id(self):
		...
		if self.product_id:
			if user.santotomascar:
				self.location_id = self.product_id.location_stc.id
			elif user.puertoquetzalcar:
				self.location_id = self.product_id.location_pq.id
			else:
